# Common config instantiation and state-dict loading helpers.
import importlib
...
def instantiate_from_config_with_arg(config: Mapping[str, Any], arg) -> object:
    if not "target" in config:
        raise KeyError("Expected key `target` to instantiate.")
    return get_obj_from_str(config["target"])(arg, **config.get("params", dict()))
